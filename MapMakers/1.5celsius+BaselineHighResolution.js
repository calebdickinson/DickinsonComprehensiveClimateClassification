...
// -------------------------------------------------------------------
// 1. LOAD & PREP using WorldClim v1 monthly 1961-1990 climate normals
// -------------------------------------------------------------------

var DELTA_C = 1.5;  // degrees Celsius to add to every monthly mean temperature
...
var monthlyClim = ee.ImageCollection(
  months.map(function (m) {
    ...
    // Monthly mean temperature (°C) from (tmax + tmin)/2; WC v1 is in 0.1°C
    var tmaxC  = ee.Image(im).select('tmax').multiply(0.1);
    var tminC  = ee.Image(im).select('tmin').multiply(0.1);
    var tmeanC = tmaxC.add(tminC).divide(2).add(DELTA_C).rename('tmeanC');  // ← add 1.5°C here
...
// Annual sums / hottest-coldest from WC monthly
...
var validMask = coldestC.gte(-20).and(hottestC.gte(15));
var AI        = P_ann.divide(PET_ann).rename('AI');

var aridBase = ee.Image(3)                 // 3 = Humid
  .where(AI.lt(0.0036), 2)                // 2 = Semihumid
  .where(AI.lt(0.0024), 1)                // 1 = Semiarid
  .where(AI.lt(0.0012), 0)                // 0 = Desert
  .rename('aridity')
  .updateMask(AI.mask())
  .updateMask(validMask);

// High-sun precip fraction (Apr–Sep) and climate overlay IDs (4=Monsoon, 5=Mediterranean)
...
var clim = aridBase
  .where(northMask.and(aridBase.neq(0)).and(HS.gte(0.8)), 4) // 4 = Monsoon
  .where(northMask.and(aridBase.neq(0)).and(HS.lt(0.4)),  5) // 5 = Mediterranean
