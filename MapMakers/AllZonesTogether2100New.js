...
// Hottest-month & coldest-month rasters (°C)
...
function monthlyClimFrom(ic_for_pr, ic_for_tmean) {
  return ee.ImageCollection(
    months.map(function(m) {
      ...
      return rainM.addBands([petM, tmeanM]).set('month', m);
    })
  );
}
...
var histHottest = monthlyClim.qualityMosaic('tmeanC').select('tmeanC').rename('histHottest');
var histColdest = monthlyClim
...
var validMask = histColdest.gte(-35).and(histHottest.gte(10));
...
// High-sun precip fraction (Apr–Sep)
...
// ===============================
// C) Your zone classifiers (unchanged)
// ===============================
