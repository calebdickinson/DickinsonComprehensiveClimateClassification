...
The conceptual foundation of the Dickinson system treats Earth’s climates
as a subset of a broader, hypothetical climate space. Let $\mathcal{C}$
denote the set of all climate regimes in which life is physically possible.
Observed Earth climates occupy a proper subset of $\mathcal{C}$, but the
classification itself is defined over the full domain. This framing does
not assert that all regions of climate space described by the system are
...
at least $-30^\circ$C. These thresholds exclude polar, permafrost-dominated,
and cold-summer climates in which potential evapotranspiration is minimal
or poorly defined and precipitation seasonality does not exert a
first-order control on climatic or ecological differentiation.

At locations that do not satisfy these thermal screening criteria, no
aridity regime is assigned and the moisture classification is considered
not applicable within the Dickinson framework.
...
totals and annual potential evapotranspiration is taken as twelve times
the climatological mean monthly PET. All aridity regimes are defined using
this dimensionless ratio, ensuring invariance under uniform rescaling of
hydrological fluxes.
...
large scales. In particular, hyper-arid environments are incorporated into
a single arid desert category, while the humid and subhumid ranges are
...
\]
This diagnostic captures hemispherically asymmetric seasonal regimes,
particularly those characterized by dry summers and wet winters.
...
minimum monthly precipitation of at least 5\% of mean monthly PET,
expressed in a dimensionally explicit form to avoid reliance on opaque
constants.
