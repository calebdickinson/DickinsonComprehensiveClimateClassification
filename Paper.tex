...
comparing climate regimes across paleoclimates, present conditions,
future projections, and hypothetical climate states.
...
and structure from widely used systems such as the Köppen classification (Köppen, 1884) or the UNEP aridity index (UNEP, 1992). The Köppen system is explicitly ecological and biogeographic, with classification thresholds calibrated to the present-day distribution of vegetation and climatic analogues on Earth. The UNEP aridity index, by contrast, is development-oriented, designed primarily to assess land
...
This framework is particularly useful for tracking climate regime transitions under strong warming, identifying emergent climates with no present-day analogues, and comparing paleoclimatic and future states within a single, invariant classification scheme.
...
applied to cold- and warm-season metrics. Cold-month thermal zones are
...
By contrast, the warm-season thermal axis is subdivided more finely, using
5~$^\circ$C intervals within each cold-month zone, to reflect the
continuous and highly sensitive role of summer warmth in governing surface
energy balance, evapotranspiration, and heat stress. Accordingly, each
cold-month thermal class is resolved into multiple warm-month
subcategories (e.g., C into c$_1$ and c$_2$, B into b$_1$ and b$_2$, and
analogously for A and Z), allowing climates with similar winter
constraints but markedly different summer regimes to be distinguished.

This asymmetric treatment follows long-standing precedent in climate
classification; for example, the Köppen system resolves multiple
summer-heat categories while employing comparatively coarser
winter-temperature thresholds (Köppen, 1884). The combined use of
10~$^\circ$C cold-month bins and finer warm-month subdivisions thus
provides a transparent, scale-consistent partition that remains robust
across paleoclimates, projected future warming, and hypothetical climate
states.
...
The term \emph{hypercaneal} is used here strictly as a thermal descriptor for extremely hot climatological monthly mean temperatures. Hypercaneal cold-month and/or warm-month thermal zones should be understood as a \emph{necessary but not sufficient} condition for the formation of hypercane vortices (Emanuel, 1988). While such extreme thermal states are a prerequisite, hypercane development also depends on additional dynamical and oceanic factors that are not represented in a monthly-mean thermal
...
at least $-30^\circ$C. Together, these warm- and cold-season constraints define a conservative thermal envelope within which aridity diagnostics exert first-order influence on climate differentiation. Outside this envelope, climates are classified exclusively on thermal grounds.
