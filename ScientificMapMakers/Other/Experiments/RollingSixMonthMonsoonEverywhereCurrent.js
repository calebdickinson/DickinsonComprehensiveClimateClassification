...
// Mediterranean + Monsoon excluded ONLY if ARID DESERT (class 1)
...
// ❗ ONLY exclude ARID DESERT (1), NOT semiarid
var medMask = (
...
// =====================
var clim = aridBase
  .where(medMask, 3)           // Mediterranean
  .where(globalMonsoon, 4)     // Monsoon
  .where(oceanMask, 8)
  .where(coldCond, 7)
  .rename('climateClass');
...
Map.addLayer(
  clim
    ...
    .remap(codes, ee.List.sequence(0, codes.length - 1)),
  { min: 0, max: codes.length - 1, palette: palette },
  'Climate (Global Monsoon, semiarid allowed)',
  true, 0.7
);
