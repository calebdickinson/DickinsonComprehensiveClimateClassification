...
// Output: Penman-equivalent PET mean (mm/month) for 2071–2100
var OUT_ASSET_ID = PREFIX + 'CHELSA_pet_penman_mean_2071-2100';
...
function fut_extC(mm){
  var idMax = PREFIX + 'CHELSA_ukesm1-0-ll_r1i1p1f1_w5e5_ssp585_tasmax_' + mm + '_2071_2100_norm';
  var idMin = PREFIX + 'CHELSA_ukesm1-0-ll_r1i1p1f1_w5e5_ssp585_tasmin_' + mm + '_2071_2100_norm';
  var rawMax = ee.Image(idMax), rawMin = ee.Image(idMin);
  var tmax = withPenmanMask(rawMax.updateMask(rawMax.neq(NODATA_U16)).multiply(0.1).subtract(273.15)).rename('tmaxC');
  var tmin = withPenmanMask(rawMin.updateMask(rawMin.neq(NODATA_U16)).multiply(0.1).subtract(273.15)).rename('tminC');
  return {tmaxC: tmax, tminC: tmin};
...
var PET_penman_mean_future = PET_harg_mean_future
  ...
  .rename('CHELSA_pet_penman_mean_2071-2100')
  .max(0);
...
Map.addLayer(PET_harg_mean_future, {min:0, max:250, palette:['#f7fbff','#6baed6','#08306b']}, 'PET mean (Harg) 2071–2100', false);
Map.addLayer(PET_penman_mean_future, {min:0, max:250, palette:['#ffffe5','#7fcdbb','#2c7fb8']}, 'PET mean (Penman-equivalent) 2071–2100', true);
...
Export.image.toAsset({
  image: PET_penman_mean_future
    ...
    .clip(WORLD_RECT),
  description: 'Export_CHELSA_pet_penman_mean_2071-2100',
