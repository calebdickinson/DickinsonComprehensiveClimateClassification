...
for (var m = 1; m <= 12; m++) {
  ...
  var tmaxAtPt = atPoint(imgMax, 'tmaxC');
  var tminAtPt = atPoint(imgMin, 'tminC');

  tasC_raw.push(tmaxAtPt.add(tminAtPt).divide(2));
}
...
// ====================================
// Extraterrestrial radiation (Ra), FAO-56, per month at LAT
// ====================================
...
var raMonthly = []; // MJ m-2 month-1
for (var i = 0; i < 12; i++) {
  var J = midMonthDOY[i];
  var dr    = 1 + 0.033 * Math.cos(2 * Math.PI * J / 365);
  var decl  = 0.409 * Math.sin(2 * Math.PI * J / 365 - 1.39);
  var omega = Math.acos(-Math.tan(latRad) * Math.tan(decl));

  var raDaily = (24 * 60 / Math.PI) * GSC * dr *
    (omega * Math.sin(latRad) * Math.sin(decl) +
     Math.cos(latRad) * Math.cos(decl) * Math.sin(omega));

  raMonthly.push(raDaily * daysInMonth[i]); // MJ m-2 month-1
}
var raMonthlyEE = ee.List(raMonthly);
...
var petAnnAtPoint  = ee.Number(petMonthlyList.reduce(ee.Reducer.sum()));
var petMeanAtPoint = petAnnAtPoint.divide(12);
