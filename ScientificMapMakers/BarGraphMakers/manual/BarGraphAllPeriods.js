...
function runForPeriod(NORMAL_PERIOD) {
  ...
  var rawLinesString = ee.String(
    combinedF.cat(combinedC).join('\n')
  );

  // ====================================
  // FORMAT METADATA AS STRINGS (PER PERIOD)
  // ====================================
  
  var annualMean = meanAnnualTemp; // alias for your formatter
  ...
  var metaLines = ee.List([
    ...
    ee.String('  const rawLines = `'),
    rawLinesString,
  
  ]);
  
  metaLines.evaluate(function(list) {
