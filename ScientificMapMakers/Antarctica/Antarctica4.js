// === CHELSA v2.1 (1981–2010) — Aridity via CHELSA methods ===
...
// === Build the discrete (indexed) class image from your 'combined' codes ===
var discrete = combined
  .remap(codes, indices)       // map e.g. 493 → 0, 593 → 1, etc.
  .rename('classIndex')
  .updateMask(combined.mask()); // keep original mask semantics

// === South of 60°S (no polygons) ===

// Mask for pixels with latitude < -60
var south60Mask = ee.Image.pixelLonLat().select('latitude').lt(-60);

// Apply to your discrete class image
var discreteSouth60 = discrete.updateMask(south60Mask);

// A simple rectangle for centering/thumbnail bounds
var south60Geom = ee.Geometry.Rectangle([-180, -90, 180, -60], null, false);

// Add to map
Map.addLayer(
  discreteSouth60,
  {min: 0, max: indices.length - 1, palette: palette},
  'Climate (south of 60°S)',
  true, 0.7
);
Map.centerObject(south60Geom, 2);

// (Optional) console thumbnail
var visImg = discreteSouth60.visualize({min: 0, max: indices.length - 1, palette: palette});
print(ui.Thumbnail(visImg, {region: south60Geom, dimensions: 3000, format: 'png'}));
