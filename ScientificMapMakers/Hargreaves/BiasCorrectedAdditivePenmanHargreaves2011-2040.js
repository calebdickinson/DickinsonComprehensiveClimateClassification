...
// --- Baseline bias: AI_penman(81–10) − AI_harg(81–10) at native 30"
var AI_BIAS = ee.Image('projects/ordinal-crowbar-459807-m2/assets/AI_diff_global_30arcsec')
  .select(0)
  .rename('AI_bias');

// Both AI (future Hargreaves) and AI_BIAS are 30". Avoid reprojecting for map tiles.
// If you ever discover a tiny misalignment, you may do:
//   var AI_on_bias = AI.resample('bilinear');   // no reproject
// but if your future grids are truly 30", you can just use AI directly.

var AI_on_bias = AI;  // keep native 30" grid

// Apply additive bias per pixel where the bias has data
var biasHasData = AI_BIAS.mask();
var AI_used = AI_on_bias
  .where(biasHasData, AI_on_bias.add(AI_BIAS))  // add bias where available
  .rename('AI_used')
  .max(0).min(100);                              // optional clamp

// ---- Lightweight display (avoid tile memory blowups)
Map.addLayer(
  AI_used.resample('bilinear'),                  // no reproject; lets EE choose pyramids
  {min: 0, max: 3},
  'AI (bias-corrected, 30")',
  false
);

// --- From here on, use AI_used (not AI) for classification:


// Ocean strictly from mask
var oceanMask = AI_used.mask().not();
...
var aridBase = ee.Image(6)                      // H: Humid
  .where(AI_used.lt(0.075), 5)                       // G: Semihumid
  .where(AI_used.lt(0.050), 2)                       // S: Semiarid
  .where(AI_used.lt(0.025), 1)                       // D: Arid Desert
  .rename('aridity');
