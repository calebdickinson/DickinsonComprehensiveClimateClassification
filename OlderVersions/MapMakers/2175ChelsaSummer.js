/***** CHELSA 2175 Summer Map (2100 + (2100−2025)) *****/
...
// ---- Build 2025, 2100, and 2175-projected tas collections ----
...
var tas2175 = ee.ImageCollection(ee.List.sequence(1,12).map(function(m){
  ...
  var t75 = t00.add(dT).rename('tasC'); // double the difference once beyond 2100
  return t75.set('month', m);
}));

// ---- Hottest & coldest month from projected 2175 temps ----
var hottestC = tas2175.qualityMosaic('tasC')
  .select('tasC').rename('hottestC');

var coldestC = tas2175.map(function(img){
...
Map.addLayer(
  ...
  'Climate (CHELSA 2175 summer: 2100 + (2100−2025))',
  true, 0.7
);

// (Optional diagnostic layer)
// Map.addLayer(hottestC, {min:0, max:45, palette:['#0011ff','#88ccff','#ffffcc','#ff9900','#cc0000']}, '2175 hottest month (°C)', false);
