/***** CHELSA monthly tas (°C) → Coldest-month classes, 2250 = 2025 + 3Δ *****/
...
// ---- build 2025, 2100, and projected 2250 stacks ----
...
var tas2250 = ee.ImageCollection(ee.List.sequence(1,12).map(function(m){
  ...
  var t50 = t00.add(dT.multiply(2)).rename('monthlyMean'); // triple the change
  return t50.set('month', m);
}));

// ---- hottest/coldest helpers ----
...
// Use the projected 2250 temps
var hottestC = hottestFromMonthly(tas2250);
var coldestC = coldestFromMonthly(tas2250);
...
// Optional: center on any known footprint
Map.centerObject(ee.Image(ASSET_ROOT + '/tas_2100_01'), 2);

Map.addLayer(
  discreteLand,
  { min: 0, max: indices.length - 1, palette: palette },
  'Climate (coldest month, 2250 projection)',
  true, 0.7
);

// Optional: show the continuous coldest-month °C
Map.addLayer(coldestC, {min:-40, max:30, palette:['#0011ff','#88ccff','#ffffcc','#ff9900','#cc0000']}, 'Coldest month (°C, 2250)', false);
