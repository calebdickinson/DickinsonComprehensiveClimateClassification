// ERA5 Monthly Mean 2m Air Temps (°C)
...
var monthlyMeans = ee.ImageCollection(
  months.map(function(m) {
    var monthly = era5.filter(ee.Filter.calendarRange(m, m, 'month')).mean();
    return monthly.set('month', m).rename('tempC');
  })
);

// Warmest and coldest monthly mean temperature per pixel
var hottestC_global = monthlyMeans.qualityMosaic('tempC').select('tempC').rename('hottestC');
...
Map.onClick(function(coords) {
  ...
  coldComb.reduceRegion({
    ...
    cLbl.setValue('Climate: ' + (coldLetters[c]||''));
  });
  warmComb.reduceRegion({
    ...
    wLbl.setValue('Summer: ' + (summerLetters[w]||''));
  });
});
